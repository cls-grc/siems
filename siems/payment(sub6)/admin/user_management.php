...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['activate_student_account'])) {
            try {
                ...
                $existingStudent = siemsFetchOne("SELECT id, full_name FROM users WHERE student_id = ? AND role = 'student' LIMIT 1", [$studentId]);
                if (!$existingStudent) {
                    throw new RuntimeException('Student profile not found. Must exist in Student Information first.');
                }

                $pdo->prepare("UPDATE users SET role = ?, password_hash = ?, is_active = ?, must_change_password = ? WHERE id = ?")
                    ->execute([$role, $passwordHash, $isActive, $mustChange, $existingStudent['id']]);
                
                siemsLogSubsystemEvent('User Management', 'Activated student account', $studentId, null, $role . ' (password set)');
                $_SESSION['message'] = "Account activated for {$existingStudent['full_name']} ($studentId). Password: $password. Login: $studentId";
...
<div class="row g-4 mb-4">
    <div class="col-lg-4">
        ...
        <div class="card-outline bg-white mb-4">
            <div class="balance-header py-3 px-4"><span class="section-title">ACTIVATE STUDENT ACCOUNT</span></div>
            <div class="card-body p-4">
                <?php $studentProfiles = siemsFetchAll("SELECT student_id, full_name FROM users WHERE role = 'student' ORDER BY full_name ASC"); ?>
                <form method="POST" class="row g-3">
                    <div class="col-12">
                        <label class="form-label row-val mb-2">Select Student Profile</label>
                        <select name="student_id" class="form-select row-text" required style="border:2px solid #e2e8f0;border-radius:8px;">
                            <option value="">Choose from Student Information profiles...</option>
                            <?php foreach ($studentProfiles as $profile): ?>
                                <option value="<?php echo htmlspecialchars($profile['student_id']); ?>"><?php echo htmlspecialchars($profile['full_name'] . ' (' . $profile['student_id'] . ')'); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-12">
                    ...
                    <div class="col-6 form-check"><input class="form-check-input" type="checkbox" name="must_change_password" checked id="force-pw-student"><label class="form-check-label row-text" for="force-pw-student">Force password change</label></div>
                    <div class="col-6 form-check"><input class="form-check-input" type="checkbox" name="is_active" checked id="active-student"><label class="form-check-label row-text" for="active-student">Active account</label></div>
                    <div class="col-12"><button type="submit" name="activate_student_account" class="btn btn-success fw-bold w-100" style="border-radius:8px;"><i class="bi bi-unlock-fill me-2"></i>Activate Account</button></div>
                </form>
            </div>
        </div>
