...
if (!in_array($module, $allowedModules, true)) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $studentTablesReady) {
        if (isset($_POST['assign_scholarship']) && !empty($_POST['student_id'])) {
            $studentId = trim($_POST['student_id']);
            $gwa = (float) ($_POST['gwa'] ?? 0);
            $scholarshipType = trim($_POST['scholarship_type'] ?? '');
            $stackable = (int) ($_POST['stackable'] ?? 1);
            $remarks = trim($_POST['scholarship_remarks'] ?? '');

            if ($gwa < 1.00 || $gwa > 5.00 || empty($scholarshipType)) {
                $_SESSION['message'] = 'Invalid GWA or scholarship type.';
                $_SESSION['msg_type'] = 'danger';
            } else {
                try {
                    $existingPending = siemsFetchOne("SELECT id FROM scholarships WHERE student_id = ? AND status = 'Pending'", [$studentId]);
                    if ($existingPending) {
                        $pdo->prepare("UPDATE scholarships SET gpa = ?, discount_type = ?, stackable = ?, remarks = ? WHERE id = ?")
                            ->execute([$gwa, $scholarshipType, $stackable, $remarks ?: null, $existingPending['id']]);
                    } else {
                        $pdo->prepare("
                            INSERT INTO scholarships (student_id, gpa, discount_type, stackable, status, assigned_at, remarks) 
                            VALUES (?, ?, ?, ?, 'Pending', NOW(), ?)
                        ")->execute([$studentId, $gwa, $scholarshipType, $stackable, $remarks]);
                    }

                    siemsLogSubsystemEvent(
                        'Student Information', 
                        'Assigned scholarship request', 
                        $studentId, 
                        null, 
                        "$scholarshipType (GWA $gwa, Stackable: " . ($stackable ? 'Yes' : 'No') . ")"
                    );

                    $_SESSION['message'] = "Scholarship '$scholarshipType' assigned (Pending). Approve in Scholarships module.";
                    $_SESSION['msg_type'] = 'success';
                } catch (Exception $e) {
                    $_SESSION['message'] = 'Failed to assign scholarship: ' . $e->getMessage();
                    $_SESSION['msg_type'] = 'danger';
                }
            }
            header('Location: student_information.php?student_id=' . urlencode($studentId) . ($search ? '&search=' . urlencode($search) : ''));
            exit;
        }
...
$password = '';
        ...
        try {
            ...
            $pdo->commit();
            $_SESSION['message'] = 'Student profile registered successfully. Login credentials managed in User Management (subsystem 10). Use password set there.';
            $_SESSION['msg_type'] = 'success';
...
if ($selectedStudentId !== '' && siemsTableExists('scholarships')) {
    $currentScholarship = siemsFetchOne("
    ...
    <div class="col-lg-8">
        <?php if ($selectedUser): ?>
            <div class="row g-4 mb-4">
                ...
                <div class="card-outline bg-white" id="scholarship-module">
                    <div class="balance-header py-3 px-4 d-flex justify-content-between align-items-center">
                        <span class="section-title">QUICK SCHOLARSHIP ASSIGNMENT</span>
                        <?php if ($currentScholarship): ?>
                            <span class="badge bg-<?php echo $currentScholarship['status'] === 'Approved' ? 'success' : ($currentScholarship['status'] === 'Rejected' ? 'danger' : 'warning'); ?> px-3 py-2">
                                <?php echo htmlspecialchars($currentScholarship['status']); ?> 
                                (<?php echo htmlspecialchars($currentScholarship['discount_type']); ?>)
                            </span>
                        <?php else: ?>
                            <span class="badge bg-secondary px-3 py-2">No Scholarship</span>
                        <?php endif; ?>
                    </div>
                    <div class="card-body p-4">
                        <form method="POST">
                            ...
                            <div class="row g-3">
                                <div class="col-md-4">
                                    <label class="form-label row-val">GWA</label>
                                    <input type="number" name="gwa" step="0.01" min="1.00" max="5.00" class="form-control row-text" placeholder="1.00-5.00" style="border: 2px solid #e2e8f0; border-radius: 8px;" required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label row-val">Scholarship Type</label>
                                    <select name="scholarship_type" class="form-select row-text" style="border: 2px solid #e2e8f0; border-radius: 8px;">
                                        <option value="Academic 50%">Academic 50% (GWA 1.00-1.75)</option>
                                        <option value="QC Foundation 100%">Foundation 100%</option>
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label row-val">Stackable</label>
                                    <select name="stackable" class="form-select row-text" style="border: 2px solid #e2e8f0; border-radius: 8px;">
                                        <option value="1">Yes</option>
                                        <option value="0">No (overrides GPA)</option>
                                    </select>
                                </div>
                                <div class="col-md-12">
                                    <label class="form-label row-val">Remarks</label>
                                    <input type="text" name="scholarship_remarks" class="form-control row-text" placeholder="Reason for scholarship assignment" style="border: 2px solid #e2e8f0; border-radius: 8px;">
                                </div>
                                <div class="col-12 mt-3">
                                    <button type="submit" name="assign_scholarship" class="btn btn-outline-success fw-bold px-5" style="border-radius: 8px; border-width: 2px;">
                                        <i class="bi bi-award me-2"></i>Assign Scholarship (Pending)
                                    </button>
                                    <?php if ($currentScholarship && $currentScholarship['status'] === 'Pending'): ?>
                                        <a href="../admin/scholarships.php" class="btn btn-warning fw-bold px-5 ms-2" style="border-radius: 8px;">
                                            <i class="bi bi-check-circle me-2"></i>Validate in Scholarships
                                        </a>
                                    <?php endif; ?>
                                </div>
                            </div>
                        ...
                        <?php if ($currentScholarship): ?>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <strong>GPA:</strong> <?php echo htmlspecialchars($currentScholarship['gpa'] ?? 'N/A'); ?><br>
                                    <strong>Discount:</strong> <?php echo ($discounts[$currentScholarship['discount_type']] ?? 0); ?>%<br>
                                    <strong>Status:</strong> <?php echo htmlspecialchars($currentScholarship['status']); ?>
                                </div>
                                <div class="col-md-6">
                                    <strong>Assigned:</strong> <?php echo $currentScholarship['assigned_at'] ? date('M j, Y', strtotime($currentScholarship['assigned_at'])) : 'N/A'; ?><br>
                                    <?php if ($currentScholarship['validated_at']): ?>
                                        <strong>Validated:</strong> <?php echo date('M j, Y', strtotime($currentScholarship['validated_at'])); ?> by <?php echo htmlspecialchars($currentScholarship['validated_by_name'] ?? 'N/A'); ?>
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endif; ?>
